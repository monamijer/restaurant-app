$(document).ready(function () {
  $(document).on("change", ".select-statut", function () {
    ...
    if (
      statut === "NO_SHOW" &&
      !confirm(
        "Confirmer le no-show ? L'acompte sera retenu et le compteur du client sera incrémenté.",
      )
    ) {
      $(this).val("");
      return;
  ...
  $(document).on("click", ".btn-confirmer-paiement", function () {
    const id = $(this).data("id");

    $.post(
      "/admin/reservations/confirmer-paiement",
      { id: id },
      function (response) {
        if (response.success) {
          afficherAlerte("success", response.message);
